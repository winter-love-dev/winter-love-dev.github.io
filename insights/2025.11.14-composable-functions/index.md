...
컴포저블 함수의 실행 결과로 생성되는 데이터는 하나의 노드(Node)로 간주되어 방출 (Emitting) 되고, 노드는 UI 를 나타내는 Composable 트리(Tree) 구조의 구성 요소로 업데이트 된다.
...
우리가 자주 부르는 단어인 ReComposition 도 이 Side effect 대목에서 다시 이해 해볼수 있다.
